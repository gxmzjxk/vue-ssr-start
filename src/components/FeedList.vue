<template>
    <div class="feed-list-wrap" ref="feed_list" data-spm="fd-d">
        <v-req source="recommend" @ready="initFetch" :empty="empty" preload="feed">
            ...
            <footer class="loading-area" v-if="!limitOnePage">
                <bottom-loading @reach="loadMore" :load_mode="load_mode" :loading="loading" :allLoaded="allLoaded">
                </bottom-loading>
            </footer>
        </v-req>
    </div>
    ...
    methods: {
        ...
        genPluginItem() {
            const feedbackItem = {
                isFeedBack: true,
                type: 0,
                title: '作为搜狐体育资深球迷，这群人你一定要认识',
                authorName: '搜狐体育',
                url: '//m.sohu.com/a/250273948_100200180',
                scm: '0.0.0.0'
            };
            const oneDay = 24 * 60 * 60 * 1000;
            let visitedFlag = JStorage.local.getItem(
                STORE_KEY.FEEDBACK_VISITED_FLAG
            );
            let vipTime = JStorage.local.getItem(STORE_KEY.BECOME_VIP_TIME);
            let viewedListStr = JStorage.local.getItem(
                STORE_KEY.DAILY_VIEWED_LIST
            );
            let viewedList = [],
                len,
                viewedDateList = [];
            let showFlag = false; // 是否展示推广 Feedback
            const nowTime = Date.now();
            const nowDate = new Moment(nowTime).formatTime('yyyyMMdd');
            if (viewedListStr) {
                viewedList = JSON.parse(viewedListStr);
            }
            len = viewedList.length;
            viewedDateList = viewedList.map(item => {
                return new Moment(item).formatTime('yyyyMMdd');
            });
            if (!visitedFlag) {
                if (vipTime) {
                    // 已经达成目标了
                    if (nowTime - vipTime < 2 * oneDay) {
                        showFlag = true;
                    }
                } else {
                    // 还未达成目标
                    if (len === 0) {
                        viewedList.push(nowTime);
                    } else {
                        if (viewedDateList.indexOf(nowDate) === -1) {
                            viewedList.push(nowTime);
                        }
                    }
                    if (viewedList.length >= 3) {
                        if ((nowTime - viewedList[0]) / oneDay <= 7) {
                            showFlag = true;
                            JStorage.local.setItem(
                                STORE_KEY.BECOME_VIP_TIME,
                                nowTime
                            );
                        } else {
                            viewedList.shift();
            ...
            // 临时方案. 如果已经加入 Session，则不再 inject
            if (showFlag && !this.preList[5].isFeedBack) {
                this.preList.splice(5, 0, feedbackItem);
            }
        },
        initFetchEnd() {
            this.genPluginItem();
